...
export class CategoryComponent implements OnInit {
  ...
  constructor(private titleService: Title
    , private notser: NotificationService, private router: Router, private loader: LoaderService,private route: ActivatedRoute, private CategoryServ: CategoryService, private dailogService: DeleteService
  ) {
  ...
  LoadCompanyName() {
    ...
    this.CategoryServ.getCategory(this.pageNumber, this.pageSize, '', this.colname, this.coldir).subscribe(response => {
      this.categoryList.push(...response?.data);
      this.categoryList.length = response?.pagination.totalCount;
  ...
  getRequestdata(pageNum: number, pageSize: number, search: string, sortColumn: string, sortDir: string) {
    ...
    this.loader.busy();;
    this.CategoryServ.getCategory(pageNum, pageSize, search, sortColumn, sortDir).subscribe(response => {
  ...
  onCreateUpdate() {
    ...
    this.Category.Id = this.form.value.Id;
    ...
    if (this.form.invalid || this.form.value.name == ' ') {
      ...
      if (this.form.value.Id == 0 || this.form.value.Id == '' ) {
        this.isDisable = true;
        this.CategoryServ.addCategory(this.Category).subscribe(res => {
          setTimeout(() => {
            this.loader.idle();
          }, 1500)
        ...
        //not used
        this.CategoryServ.updateCategory(this.Category).subscribe(res => {
          setTimeout(() => {
            this.loader.idle();
          }, 1500)
  ...
  updateEdit(row: any) {
    ...
    this.CategoryServ.updateCategory(CategoryEdit).subscribe(res => {
      if (res.status == true) {
        setTimeout(() => {
          this.loader.idle();
        }, 1500)
  ...
  getRequestdataNext(cursize: number, pageNum: number, pageSize: number, search: string, sortColumn: string, sortDir: string) {
    ...
    this.CategoryServ.getCategory(pageNum, pageSize, search, sortColumn, sortDir).subscribe(res => {
      if (res.status == true) {

        this.categoryList.length = cursize;
        this.categoryList.push(...res?.data);
        this.categoryList.length = res?.pagination.totalCount;
        this.dataSource = new MatTableDataSource<any>(this.categoryList);
        this.dataSource._updateChangeSubscription();
        this.dataSource.paginator = this.paginator as MatPaginator;
        this.loader.idle();
  ...
  onChecknameIsalreadysign() {
    this.Category.Name = this.form.value.Name;
    this.Category.Id = this.form.value.Id;
    this.CategoryServ.categoryIsalreadysign(this.Category.Name, this.Category.Id).subscribe(
      res => {
        if (res.status == true) {
  ...
  onChecknameIsalreadysignWhenUpdate(row: any) {
    ...
    this.CategoryServ.categoryIsalreadysign(CategoryName, CategoryId).subscribe(
      res => {
        if (res.status == true) {
          this.isDisabled = false;
          this.isNameUpdatedRepeated = false;
  ...
  onDelete(r: any) {
    ...
    this.dailogService.openConfirmDialog().afterClosed().subscribe(res => {
      if (res) {
        this.CategoryServ.deleteCategory(r.id).subscribe(
          rs => {
