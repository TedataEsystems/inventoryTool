...
export class IncomingComponent implements OnInit {
 ...
 // show: boolean = false;
  ...
  LoadCompanyName() {
    ...
    this.RecievedStatusServ.getRecviedStatus(this.pageNumber, this.pageSize, '', this.colname, this.coldir).subscribe(response => {
  ...
  getRequestdata(pageNum: number, pageSize: number, search: string, sortColumn: string, sortDir: string) {
    ...
    this.loader = true;
    this.RecievedStatusServ.getRecviedStatus(pageNum, pageSize, search, sortColumn, sortDir).subscribe(response => {
  ...
  onCreateUpdate() {
    ...
    if (this.form.invalid || this.form.value.name == ' ') {
      ...
      if (this.form.value.Id == 0 || this.form.value.Id == '' ) {
        ...
        //not used
        this.RecievedStatusServ.UpdateReceviedStatus(this.Recevied).subscribe(res => {
          setTimeout(() => {
            this.loader = false;
          }, 1500)
  ...
  updateEdit(row: any) {
    ...
    this.RecievedStatusServ.UpdateReceviedStatus(RecievedStatusEdit).subscribe(res => {
      if (res.status == true) {
        setTimeout(() => {
          this.loader = false;
        }, 1500)
  ...
  getRequestdataNext(cursize: number, pageNum: number, pageSize: number, search: string, sortColumn: string, sortDir: string) {
    ...
    this.RecievedStatusServ.getRecviedStatus(pageNum, pageSize, search, sortColumn, sortDir).subscribe(res => {
      if (res.status == true) {
  ...
  onChecknameIsalreadysign() {
    this.Recevied.Name = this.form.value.Name;
    this.Recevied.Id = this.form.value.Id;
    this.RecievedStatusServ.ReceviedStatusIsalreadysign(this.Recevied.Name, this.Recevied.Id).subscribe(
      res => {
        if (res.status == true) {
  ...
  onChecknameIsalreadysignWhenUpdate(row: any) {
    ...
    this.RecievedStatusServ.ReceviedStatusIsalreadysign(recievedName, recievedId).subscribe(
      res => {
        if (res.status == true) {
          this.isDisabled = false;
          this.isNameUpdatedRepeated = false;
